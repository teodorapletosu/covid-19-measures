...
(function() {
  ...
  sk.createSankey = function(containerId, configSankey, dataSankey) {
    ...
    var _safeValueToLink = function(v) {
      return Math.max(v, Number.MIN_VALUE);
    };
    ...
    dataSankey.nodes.map(function(d) {
      /// console.debug(d.sourceLinks, configSankey.nodes.sourceColors(d.sourceLinks.length));
      /// // return d.color;
      /// if (d.sourceLinks.length > 0) {
      ///   return configSankey.nodes.sourceColors(d.sourceLinks.length);
      /// }
      /// return configSankey.nodes.targetColors(d.targetLinks.length);
    ...
    var svg_base = container
      ...
      .attr(
        "width",
        dimensions.width + configSankey.margin.left + configSankey.margin.right
      )
      .attr(
        "height",
        dimensions.height + configSankey.margin.top + configSankey.margin.bottom
      )
      .attr("class", "sk-svg");
    var svg = svg_base
      .append("g")
      .attr(
        "transform",
        "translate(" +
          configSankey.margin.left +
          "," +
          configSankey.margin.top +
          ")"
      );

    var sankey = d3
      .sankey()
      .nodeWidth(configSankey.nodes.width)
      .nodePadding(10)
    ...
    if (configSankey.nodes.dynamicSizeFontNode.enabled) {
      ...
      _updateRangeFontData = function(nodeData) {
        _dynamicFontSize.domain(
          d3.extent(nodeData, function(d) {
            return d.value;
          })
        );
      };
    ...
    if (configSankey.nodes.draggableX != undefined)
      _nodes_draggableX = configSankey.nodes.draggableX;
    if (configSankey.nodes.draggableY != undefined)
      _nodes_draggableY = configSankey.nodes.draggableY;

    //Colors
    //set color in nodes, case not exists
    if (configSankey.nodes.colors) {
      for (var i = 0; i < _dataSankey.nodes.length; i++)
        if (_dataSankey.nodes[i].color == undefined)
          _dataSankey.nodes[i].color = configSankey.nodes.colors(
            _dataSankey.nodes[i].name.replace(/ .*/, "")
          );
    }

    //Tooltip function:
    //D3 sankey diagram with view options (Austin CzarneckiÃÂ¯ÃÂ¿ÃÂ½s Block cc6371af0b726e61b9ab)
    ...
    tipNodes.html(function(d) {
      ...
      for (i = 0; i < linksFrom.length; ++i) {
        html +=
          "<tr>" +
          '<td class="col-left">' +
          linksFrom[i].source.name +
          "</td>" +
          '<td align="right">' +
          _formatValueTooltip(linksFrom[i].value) +
          "</td>" +
          "</tr>";
      }
      if ((linksFrom.length > 0) & (linksTo.length > 0)) {
        html +=
          "<tr><td></td><td></td><tr><td></td><td></td> </tr><tr><td><h2>" +
      ...
      for (i = 0; i < linksTo.length; ++i) {
        html +=
          "<tr>" +
          '<td class="col-left">' +
          linksTo[i].target.name +
          "</td>" +
          '<td align="right">' +
          _formatValueTooltip(linksTo[i].value) +
          "</td>" +
          "</tr>";
      }
      html += "</table></div>";
    ...
    tipNodes.move = function(event) {
      ...
      tipNodes
        .style("top", function() {
          if (
            d3.event.pageY - topContentSVG - tipNodes.boxInfo.height - 20 >=
            0
          )
    ...
    if (configSankey.links && configSankey.links.onClick) {
      link.on("mousedown", function(e) {
        const { source, target } = e;
        const dataSource = dataSankey.nodes[source.index];
        const dataTarget = dataSankey.nodes[target.index];
        return configSankey.links.onClick(
          { source: dataSource, target: dataTarget },
          e,
          this
        );
      });
    ...
    function _dragmove(d) {
      ...
      if (_nodes_draggableX && d3.event.x < dimensions.width) {
        d.x0 = Math.max(
          0,
          Math.min(dimensions.width - sankey.nodeWidth(), d.x0 + d3.event.dx)
        );
      ...
      if (_nodes_draggableY && d3.event.y < dimensions.height) {
        ...
        d.y0 = Math.max(
          0,
          Math.min(dimensions.height - (d.y1 - d.y0), d.y0 + d3.event.dy)
        );
    ...
    if (configSankey.nodes && configSankey.nodes.onClick) {
      node.on("mousedown", function(e) {
        const data = dataSankey.nodes[e.index];
        return configSankey.nodes.onClick(data, e, this);
      });
    }
    //Drag nodes
    ...
    this.updateData = function(dataUpdated) {
      for (var i = 0; i < dataUpdated.links.length; i++) {
        var idLinkUpdate =
          dataUpdated.links[i].id ||
          dataUpdated.links[i].source + " -> " + dataUpdated.links[i].target;
