// jshint ignore: start
/*global $, d3, _, accounting, chroma, clustering, numeral, block, unblock*/
const COLUMN_FOR_COUNTRY = "Country";
const COLUMN_FOR_GLOBAL_BUDGET = "Global public budget";
const COLUMN_FOR_CULTURE_BUDGET = "Public budget for culture";
const COLUMN_FOR_INDUSTRY_BUDGET = "Cultural Industry private budget";
const COLUMN_FOR_PUBLIC_RATIO = "%public budget for culture out of total";
const COLUMN_FOR_PRIVATE_RATIO = "%private budget out of public";
...
const decodeBudget = input => {
  ...
  if (rawBudged) {
    ...
    // console.debug(">> decoded", rawBudged, decodedBudgetValue);
  }
...
const renderTooltip = d => {
      ...
      <table>
        ...
        <% budgets.forEach(function(budget) { %>
          <tr>
            ...
            <td data-source="value">
              <span><%- budget.value %></span>
              <% if (withDetails) { %>
              <strong><%- budget.detail %></strong>
              <% } %>
            </td>
          </tr>
        <% }) %>
  ...
  // console.debug(">> decode label", d[COLUMN_FOR_COUNTRY]);
  const html = render({
...
$(document).ready(() => {
  ...
  fetchDataset()
    .then(ctx => {
      ...
      const items = ctx.dataSet.sheets.Budgets;

      let preparedItems = JSON.parse(JSON.stringify(items.rows)).map(it => {
        // console.debug(">> decode", it[COLUMN_FOR_COUNTRY]);
        it[COLUMN_FOR_PUBLIC_RATIO] = decodeBudget(it[COLUMN_FOR_PUBLIC_RATIO]);
        it[COLUMN_FOR_PRIVATE_RATIO] = decodeBudget(
          it[COLUMN_FOR_PRIVATE_RATIO]
        );
      ...
      const records = context.dataSet.sheets.Budgets.rows;
      const countries = records.map(it => it[COLUMN_FOR_COUNTRY]);

      window.reset = () => {
        $(".svgDiagram").empty();
        setupGraph(countries, preparedItems, factorX, factorY, records);
        setupTooltips(records);
      };
      window.reset();
...
const getCountryColors = (data, logScaleX, xRange, logScaleY, yRange) => {
  ...
  const clustersColors = chroma
    .scale(["#6DE9D2", "#BB32F0", "#96B0EE", "#F0EA3D"])
    .mode("lch")
...
const setupGraph = (countries, items, factorX, factorY, originalDataset) => {
  ...
  const margin = { top: 28, right: 25, bottom: 100, left: 95 };
  ...
  const svgDefs = svgStage.append("svg:defs");
  // font
  svgDefs
    .append("style")
    .attr("type", "text/css")
  ...
  const plot = input => {
    ...
    const logScaleX = [0.0001, logScaleXMax - 0.0001];
    ...
    const logScaleY = [0.0001, logScaleYMax - 0.0001];
